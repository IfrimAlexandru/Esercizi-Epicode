...
public class ProvaOrdini {
    ...
    public static void main(String[] args) {
        ...
        Map<Cliente, Double> m2 = ordini.stream().collect(Collectors.groupingBy(Ordine::getCliente,
                Collectors.summingDouble(value -> value.getProdotti().stream().mapToDouble(Prodotto::getPrezzo).sum())));
        ...
        Collectors.minBy(Comparator.comparingDouble(Prodotto::getPrezzo).reversed())));
        m3.forEach((categoria, prodottoMax) -> System.out.println(categoria + " -> " + prodottoMax.get()));

        //Esercizio4
        System.out.println("-----Esercizio4-----");
        OptionalDouble mediaImporti = ordini.stream()
                .mapToDouble(ordine -> ordine.getProdotti().stream()
